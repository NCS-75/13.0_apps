odoo.define('website_purchase_quote.website_purchase_quote', function (require) {
'use strict';
...
if(!$('.o_website_quote').length) {
    ...
    var UpdateLineButton = Widget.extend({
        ...
        onClick: function(ev){
            ...
            ajax.jsonRpc("/purchase/update_line", 'call', {
                'line_id': line_id[1],
                ...
                $('[data-id="total_amount_purchase"]>span').html(data[1]);
            });
            return false;
        },
    });
    ...
    var NavigationSpyMenu = Widget.extend({
        ...
        generateMenu: function(){
            var self = this;
            // reset ids
            $("[id^=purchase_header_], [id^=purchase_]", this.spy_watched).attr("id", "");
            ...
            _.each(this.spy_watched.find("h1, h2"), function(el){
                ...
                switch (el.tagName.toLowerCase()) {
                    case "h1":
                        id = self.setElementId('purchase_header_', el);
                        ...
                        break;
                    case "h2":
                        id = self.setElementId('purchase_', el);
                        text = self.extractText($(el));
                        if (!text) {
                            break;
                        }
    ...
    nav_menu.setElement($('[data-id="purchase_sidebar"]'));
    nav_menu.start($('body[data-target=".navspy1"]'));
